...
def cross_validate_and_save_models(config, X_train_array, Y_train_array, model_name, model_subfolder, Nfold, get_model, train_model, evaluate_model, best_params):
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(skf.split(X_train_array, Y_train_array)): #  for the hard coded line!!!!!!!!!!!!!!!!!!!!!!!
        ...
        metrics, ypred_i, yprob_i = evaluate_model(fold_model_path, CrossVal_data_test, CrossVal_label_test)
...
'''def cross_validate_and_save_models(config, X_train_array, Y_train_array, model_name, model_subfolder, Nfold, get_model, train_model, evaluate_model, best_params):
    ...
    #####   New  #######
    import pickle
    cv_path = r"D:\0000-UHN\PGK2\Code\HyperparameterTuning\cv-agglo-tanimoto-test100-clusterheldout\cv_splits.pkl"
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(cv_splits):
        CrossVal_data_train = X_train_array[train_idx]
        CrossVal_data_test = X_train_array[test_idx]
        CrossVal_label_train = Y_train_array[train_idx]
        CrossVal_label_test = Y_train_array[test_idx]

        # =====================================================
        # Optional downsampling of train LABEL=0
        # If train_neg_ratio is NaN -> do nothing
        # If integer -> keep all positives and downsample negatives
        # =====================================================
        #train_neg_ratio= np.nan
        train_neg_ratio = 8
        if not pd.isna(train_neg_ratio):
            ...
            n_neg_target = int(train_neg_ratio * n_pos)

            # only downsample if needed
            if len(neg_local_idx) > n_neg_target:
                sampled_neg_local_idx = rng.choice(
                    neg_local_idx,
                    size=n_neg_target,
                    replace=False
                )
        ...
        # =====================================================
        # Save this fold's metrics next to its model
        # =====================================================
        fold_metrics_path = os.path.join(
            model_subfolder,
            f"model_fold{fold_idx + 1}_metrics.csv"
        )

        if isinstance(metrics, dict):
            metrics_row = metrics.copy()
        else:
            metrics_row = {"metrics": metrics}

        metrics_row["fold"] = fold_idx + 1
        metrics_row["train_size"] = len(CrossVal_label_train)
        metrics_row["train_pos"] = int(np.sum(CrossVal_label_train == 1))
        metrics_row["train_neg"] = int(np.sum(CrossVal_label_train == 0))
        metrics_row["test_size"] = len(CrossVal_label_test)
        metrics_row["test_pos"] = int(np.sum(CrossVal_label_test == 1))
        metrics_row["test_neg"] = int(np.sum(CrossVal_label_test == 0))

        pd.DataFrame([metrics_row]).to_csv(fold_metrics_path, index=False)
    #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    #####   New end  #######
