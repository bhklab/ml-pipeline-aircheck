...
class MLflowModelRegistration:
    ...
    def load_model_from_path(self, model_path: str, model_type: str):
        ...
        try:
            ...
            if file_extension == '.pkl':
                ...
                logger.info(
                    f"Successfully loaded pickle model from {model_path}")
                return model
        ...
        for col in metric_columns:
            if col in row.index and pd.notna(row[col]):
                try:
                    ...
                    logger.warning(
                        f"Could not convert {col} to float: {row[col]}")
    ...
    def register_model(self, row: pd.Series, experiment_name: str = "CSV_Model_Import", model_path: str = "", run_name: str | None = None) -> bool:
        """
        ...
        try:
            ...
            if file_extension not in ['.pkl', '.h5']:
                logger.info(
                    f"Skipping unsupported file type: {file_extension}")
                return False
            ...
            parent_run_name = parent_run_name.replace(
                ' ', '_').replace('.', '_')

            # Start parent run for this model
            logger.info(
                f"Starting MLflow run for model: {row.get('ModelType', 'unknown')}")
            ...
            with mlflow.start_run(run_name=parent_run_name) as parent_run:
                ...
                if file_extension == '.pkl':
                            ...
                            if hasattr(clf, 'get_params'):
                                ...
                                for param_name, param_value in model_params.items():
                                    if param_value is not None:
                                        mlflow.log_param(
                                            f"model_{param_name}", str(param_value))

                            # Log the model
                            mlflow.sklearn.log_model(
                                clf, "model", registered_model_name=None)
                    ...
                    with mlflow.start_run(run_name=f"{run_name}_keras", nested=True) as nested_run:
                        try:
                            ...
                            mlflow.log_param("model_type", "keras_sequential" if isinstance(
                                model, tf.keras.Sequential) else "keras_functional")

                            # Log model architecture info
                            mlflow.log_param(
                                "total_params", model.count_params())
                            mlflow.log_param("trainable_params", sum(
                                [tf.keras.backend.count_params(w) for w in model.trainable_weights]))
                            mlflow.log_param("non_trainable_params", sum(
                            ...
                            if hasattr(model, 'optimizer') and model.optimizer is not None:
                                ...
                                if hasattr(model.optimizer, 'learning_rate'):
                                    mlflow.log_param("learning_rate", float(
                ...
                logger.info(
                    f"Successfully registered model: {run_name}, Parent Run ID: {parent_run_id}")
                return True

        except Exception as e:
            logger.error(
                f"Error registering model from row {row.name}: {str(e)}")
            return False

    def process_csv(self, csv_path: str, experiment_name: str = "CSV_Model_Import") -> Dict[str, int]:
        """
        ...
        try:
            ...
            for index, row in df.iterrows():
                ...
                model_path = f"{folder_name}/BestModels/"
                ...
                for entry in os.listdir(model_path):
                    ...
                    if os.path.isfile(entry_path) and (entry.endswith(".pkl") or entry.endswith(".h5")):
                        ...
                        logger.warning(
                            f"No files found in model path: {model_path}")
                        results['skipped'] += 1
                        raise FileNotFoundError(
                            f"File not found or unsupported format: {entry_path}")

                run_name = entry.split(".")[0]

                if self.register_model(row, experiment_name, model_path=target_files, run_name=run_name):
                    results['success'] += 1
                else:
                    results['failed'] += 1
