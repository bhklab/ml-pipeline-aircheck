    ...
    def _load_best_models(self) -> pd.DataFrame:
        ...
        if not os.path.exists(self.best_models_path):
            raise FileNotFoundError(
                f"Required file not found: {self.best_models_path}")
    ...
    def predict_ensemble(
        ...
        for i, row in models_info.iterrows():
            ...
            if os.path.isdir(model_path):
                ...
                if not found:
                    raise FileNotFoundError(
                        f"No supported model file (.pkl or .h5) found in {model_path}")
...
class ScreeningPipeline:
    ...
    def calculate_screening_probabilities(
        ...
        if self.model_manager is None:
            raise ValueError(
                "Model manager not initialized - BestModelsResults.csv not found")
    ...
    def _process_screen_file(
        ...
        process_file(screen_path, output_file,
                     self.config.desired_columns, self.config.smiles_column)
        ...
        X_screen = DataProcessor.convert_columns_to_array(
            X_screen, self.config.desired_columns)
        ...
        self._store_individual_results(
            screen_results, all_probs, all_pred_sets)
    ...
    def _store_individual_results(
        ...
        if self.config.conformal_prediction:
            for i, y_pred_set in enumerate(all_pred_sets):
                screen_results[f"model_{i}_conformal_set"] = [
                    row for row in y_pred_set]

    def apply_chemistry_filters(self) -> None:
        """Apply chemistry filters to screening results."""
        for screen_path in self.config.screen_data:
            base_name = os.path.splitext(os.path.basename(screen_path))[0]
            screen_result_path = self.run_folder_path / \
                f"{base_name}_screen.csv"
            ...
            nominees_filtered = pd.merge(
                nominees, filter_df, left_index=True, right_index=True)

            # Save all filter results
            nominees_filtered_path = self.run_folder_path / \
                f"{base_name}_screen_ChemistryFiltersResults.csv"
            ...
            after_filters_path = self.run_folder_path / \
                f"{base_name}_screen_AfterChemistryFilters.csv"
    ...
    def _process_clustering(
        ...
        df_fp = df_fp[
            df_fp[self.config.smiles_column].isin(
                nominees_filtered[self.config.smiles_column])
        ].reset_index(drop=True)

        df_fp = DataProcessor.convert_columns_to_array(
            df_fp, self.config.desired_columns)
        ...
        fps = [DataStructs.CreateFromBitString(
            "".join(map(str, fp.astype(int)))) for fp in df_fp[fp_column]]
        ...
        if "average_yprob" in nominees_filtered.columns:
            nominees_filtered.sort_values(
                by=["average_yprob"], ascending=False, inplace=True)
            best_nominees = nominees_filtered.groupby(
        ...
        output_path = self.run_folder_path / \
            f"{base_name}_screen_Clustered.csv"
