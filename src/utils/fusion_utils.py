...
class ModelSelector:
    ...
    def __init__(self, config: Dict[str, Any], run_folder_name: str):
        ...
        self.trainfile_for_modelselection = config.get("trainfile_for_modelselection")
        self.evaluationfile_for_modelselection = config.get("evaluationfile_for_modelselection")
    ...
    def _sort_models(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.evaluation_column and all(col in df.columns for col in self.evaluation_column):
            df_sorted = df.sort_values(by=self.evaluation_column, ascending=[False] * len(self.evaluation_column))
            ...
            df_sorted = df.sort_values(by=self.crossvalidation_column, ascending=[False] * len(self.crossvalidation_column))
        else:
    ...
    def select_best_models(self) -> pd.DataFrame:
        ...
        if not self.evaluation_column:
            raise ValueError("You must provide evaluation_column in the configuration")
        ...
        print(f"Top {self.num_top_models} distinct model paths written to {self.output_txt_path}")
...
class ModelFusionPipeline:
    ...
    def _process_test_file(self, test_path: str, model_files: List[Path]) -> Dict[str, Any]:
        ...
        for model_file in model_files:
            ...
            column_name = model_name.split("_")[-1]  # Assuming column name is the last part
            X_test_array = np.stack(X_test[column_name])

            # Load and predict with model
            model = self._load_model(model_file)
            y_pred, y_proba = self._predict_with_model(model, X_test_array, model_file)
        ...
        metrics =ModelEvaluator.calculate_metrics(X_test_array, Y_test_array, y_pred_fusion, y_proba_fusion)
